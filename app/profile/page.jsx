    ...
    <section className="mt-5 minheight" style={{ width: "90vw" }}>
      ...
      <div className="w-full mt-10">
        <div className="mx-auto text-slate-600 font-semibold flex justify-evenly md:w-1/4 w-3/4">
          ...
          <div>
            <button className=" text-lg hover:text-slate-900" onClick={() => setType("completed")}>Completed</button>
            {type === "completed" ? <hr className="h-1 bg-gray-700 border-0 rounded " /> : null}
          </div>
        </div>
        <div>
          {type === "joined" ? (
            <RideCardList
              ...
              joined={true}
            />
          ) : type === "created" ?(<RideCardList data={createdRides}
          joined={false}
          handleDelete={handleDelete}
          handleEdit={handleEdit}
        />): (

            <RideCardList data={completedRides}
              joined={true}
              handleDelete={handleDelete}
              handleEdit={handleEdit}
            />
